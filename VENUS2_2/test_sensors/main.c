...
#define BLACK_TOTAL 1000   // total R+G+B below this = BLACK
...
static const char *classify(uint32_t r, uint32_t g, uint32_t b) {
    ...
    if (total < BLACK_TOTAL) return "BLACK";
    if (total > 5000) return "WHITE";

    // Each channel's percentage of the total (0-100)
    int rp = (100 * r) / total;
    int gp = (100 * g) / total;
    int bp = (100 * b) / total;

    // White: all three reasonably balanced
    if (rp > WHITE_PCT && gp > WHITE_PCT && bp > WHITE_PCT) return "WHITE";

    // Otherwise: whichever channel dominates
    if (rp >= gp && rp >= bp) return "RED";
    if (gp >= rp && gp >= bp) return "GREEN";
    if (bp >= rp && bp >= gp) return "BLUE";
    return "?";
}
