...
#define PIPE_IN  "ipc:///tmp/R2_info.ipc"
...
int main(void) {  
  ...
  char buf[1024];
  double forward_steps = 0;
  double angle = 0;
  int speed = 0;
  bool scanning = false;
  bool scanning_last = false;
  ...
  printf("Format: [forward_steps] [angle] [speed_ticks]\n");
  ...
  while (1) {    
    ...
    if (n >= 0) {
      ...
      if (!from_alg) {
          ...
          scanning_last = scanning;
          
          angle = cJSON_GetObjectItem(from_alg, "turn_angle")->valuedouble;
          speed = cJSON_GetObjectItem(from_alg, "speed")->valueint;
          scanning = cJSON_IsTrue(cJSON_GetObjectItem(from_alg, "scanning"));
        
//        printf("[MOT] Received — angle=%f  speed=%d  scanning=%d\n", angle, speed, scanning);
    ...
    if (forward_steps != 0 || angle != 0 || scanning != scanning_last) {
      
      // --- INTERCEPT TERMINAL SHORTCUT FIRST ---
      if (scanning != scanning_last) {
        execute_25_degree_twitch();
        
        forward_steps = 0;
        angle = 0;
        base_speed = 30000; // Force high-speed baseline
        scanning_last = scanning;
      }
      ...
      if (l_speed < 30000 && (forward_steps != 0 || angle != 0)) {
        l_speed = 30000; 
        r_speed = 30000;
      }
      ...
      if (fabs(forward_steps) < 0.01 && fabs(angle) > 0.01) {
        l_steps = (int16_t)(-angle * 1255);
        r_steps = (int16_t)(angle * 1255);
        printf(">> Spinning in place! Target Steps: L=%d, R=%d\n", l_steps, r_steps);
      }
      // B. Sequential 180-degree Turn AND Drive Forward
      else if (forward_steps < -0.1 && fabs(angle) < 0.2) {
        ...
        double forward_drive = fabs(forward_steps); 
        l_steps = (int16_t)((forward_drive - angle) * 1600);
        r_steps = (int16_t)((forward_drive + angle) * 1600);
      } 
      // C. Regular Driving & Banking Logic
      else {
        l_steps = (int16_t)((forward_steps - angle) * 1600);
        r_steps = (int16_t)((forward_steps + angle) * 1600);
      }

      // --- DIRECTION FIX ---
      if (base_speed < 0 && !(forward_steps < -0.1 && fabs(angle) < 0.2)) {
        l_steps = -l_steps;
        r_steps = -r_steps;
        printf(">> Reverse detected! Inverting steps.\n");
      }

      if (forward_steps == 0 && angle == 0 && base_speed == 0) {
