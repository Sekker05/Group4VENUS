...
int main() {
    ...
    zmq_setsockopt(sub_sen, ZMQ_CONFLATE, &conflate, sizeof(conflate));
    ...
    zmq_setsockopt(sub_alg, ZMQ_CONFLATE, &conflate, sizeof(conflate));
    ...
    // Variables regarding other modules
    char buf[1024];
    // Sensors
//    int  distance    = -1;
//    int  temperature = -1;
...
//
//                printf("[EMB] From sensors — distance=%d  temp=%d  front=%s\n",
//                       distance, temperature, color_front);
                fflush(stdout);
        ...
        // TODO: Algorithm already gets sensor data from sen2emb line, send necessary information
...
//            if (!from_alg) {
//                printf("[EMB] Bkad JSON from algorithm\n");
//            } else {
//                // TESTING
...
//                // TESTING
//                cJSON_AddNumberToObject(to_com, "distance", 10);
//
